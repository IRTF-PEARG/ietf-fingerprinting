...
and so the earliest results achieved sucess on SSL/TLS in this model. 
\footnote{
For a realistic attack, \adv\ would need to
monitor \emph{every} possible page of interest to each client, which is impractical.}
Attacks against proxying privacy technologies such as VPNs and SSH tunneling,
...
\noindent
{\bf (1) Closed-world WF on SSL/TLS} 
...
\marginpar{[TW] I think one of the recent neural networks papers is also on SSL/TLS? But it's actually open-world?}
...
is therefore limited if endpoints pad packets to the MTU or another fixed length. \marginpar{[TW] Can you describe on which technologies this would be easy to do? I know Tor does it, but I don't know if it's easy to pad packets to MTU on, say, TLS 1.X, QUIC, HTTP/2, or HTTP/3.}
It has been shown that these attacks are not successful on Tor. 
...
\noindent
{\bf (2) Closed-world WF on Tor}

Shmatikov and Wang \cite{shmatikov2006timing}
...
\noindent
{\bf (3) Open-world WF on Tor}

Panchenko et al. \cite{panchenko2011website}
