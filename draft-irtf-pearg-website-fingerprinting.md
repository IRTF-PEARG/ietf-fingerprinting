...
informative:
  gong2020zero:
    ...
    target: https://www.usenix.org/system/files/sec20summer_gong_prepub.pdf
    authors:
      -
        ins: Tao Wang
        org: Department of Computer Science and Engineering, Hong Kong University of Science and Technology
      -
        ins: Jiajun Gong
        org: Department of Computer Science and Engineering, Hong Kong University of Science and Technology
  CRIME:
...
towards encrypting the TLS SNI extension {{?I-D.ietf-tls-esni}}, help minimize metadata
...
- Gong et al. developed {{gong2020zero}} is a lightweight defense that does not delay any packets, minimizing its effect on
...
TLS 1.3, for example, supports record-layer padding {{RFC8446}}, although it is
...
ossificatiion and privacy enhancement. IPsec Traffic Flow Confidentiality {{?RFC4303}}
and Traffic Flow Security {{?I-D.ietf-ipsecme-iptfs}} are two mechanisms by which
endpoints can ESP datagrams to mask size metadata.

Future protocols should follow these trends when possible to remove unnecessary metadata from
the network.
